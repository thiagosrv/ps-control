...
export function ReportsPage() {
  ...
  const form = useForm<ReportFilterValues>({
    ...
    defaultValues: { name: '', cpf: '', rg: '', plate: '', visitor_type: '', date_from: '', date_to: '' },
  })
  ...
  return (
    <div>
      <PageHeader
        title="Relatórios"
        description="Consulte e exporte registros de visitas"
        action={
                ...
                {results.length === 0 ? (
                  ...
                  results.map((v) => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="text-sm">{v.visitor?.cpf ? formatCPF(v.visitor.cpf) : '—'}</TableCell>
                      <TableCell className="text-sm">{v.company_user?.full_name ?? '—'}</TableCell>
                      <TableCell><Badge variant="outline" className="text-xs">{formatVisitorType(v.visitor_type)}</Badge></TableCell>
                      ...
                      <TableCell className="font-mono text-sm">{v.vehicle_plate ?? '—'}</TableCell>
                      <TableCell>
